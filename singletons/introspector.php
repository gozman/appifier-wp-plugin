...
class JSON_API_Introspector {
  ...
  public function get_appifier_menu() {
    $wp_appifier_menu_items = wp_get_nav_menu_items('appifier');
    $menu = array();
    foreach ($wp_appifier_menu_items as $wp_menu_item) {
      $menu_items[] = $this->get_menu_item_object($wp_menu_item);
    }
  ...
  protected function get_menu_item_object($wp_menu_item) {
	if(!$wp_menu_item) {
		return null;
	}
	return new JSON_API_Menu_Item($wp_menu_item);
  }
